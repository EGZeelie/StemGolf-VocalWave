...
import { PodcastProject, GenerationStatus, ProductionSettings, Chapter, DistributionMetadata, CreatorProfile, BlogPost, Series, YoutubeMetadata, EqualizerSettings } from '../types';
...
const DEFAULT_PROD_SETTINGS: ProductionSettings = {
    introMusic: 'none',
    outroMusic: 'none',
    playbackSpeed: 1.0,
    pitch: 0,
...
const Studio: React.FC<StudioProps> = ({ 
  ...
  // Production State
  const [prodSettings, setProdSettings] = useState<ProductionSettings>(initialProject?.productionSettings || DEFAULT_PROD_SETTINGS);
                          ...
                          <div className="col-span-12 md:col-span-4 bg-[#181818] rounded-xl border border-[#333] p-4 flex flex-col relative overflow-hidden">
                              ...
                              <div className="flex justify-between items-end h-48 px-2">
                                  {['Low', 'Mid', 'High'].map((band) => {
                                      const key = band.toLowerCase() as keyof EqualizerSettings;
                                          ...
                                          <div key={band} className="flex flex-col items-center gap-3 h-full group">
                                              <div className="flex-1 relative w-8 bg-[#111] rounded-full border border-[#272727] overflow-hidden">
                                                  ...
                                                  <input 
                                                      ...
                                                      min="-12" max="12" 
                                                      value={val}
                                                      onChange={(e) => setProdSettings(s => ({ ...s, eqSettings: { ...s.eqSettings, [key]: parseFloat(e.target.value) } as EqualizerSettings }))}
